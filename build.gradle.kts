...
// local.properties to this repo's own .android-sdk path. The installer is
// pure Kotlin/JVM — no bash/cmd shell-outs — so the same code path works on
// macOS, Linux, and Windows CI runners.
...
kotlin {
    ...
    val xcf = XCFramework("UnicodeWidth")
    ...
    swiftExport {
        moduleName = "UnicodeWidth"
        flattenPackage = "io.github.kotlinmania.unicodewidth"
    }

    android {
        namespace = "io.github.kotlinmania.unicodewidth"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "unicode-width-kotlin", version.toString())

    pom {
        name.set("unicode-width-kotlin")
        description.set("Kotlin Multiplatform port of unicode-rs/unicode-width - Determine displayed width of `char` and `str` types according to Unicode Standard Annex #11 rules")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/unicode-width-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/unicode-width-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/unicode-width-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/unicode-width-kotlin.git")
        }
    }
}
...
val fullTargetBuildTaskNames = setOf(
    ...
    "assembleUnicodeWidthXCFramework",
)
