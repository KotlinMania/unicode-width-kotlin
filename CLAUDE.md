# Claude Code Project Instructions — unicode-width-kotlin

## Project Overview

This is **unicode-width-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`unicode-width`](https://crates.io/crates/unicode-width).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/unicode-width/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/unicode-width>
