...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `UnicodeWidth.swiftmodule/`
...
//      `import UnicodeWidth` above would fail at compile time.
//
//   2. The static archive `libUnicodeWidth.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "UnicodeWidth" }` and
//      `flattenPackage = "io.github.kotlinmania.unicodewidth"` configuration in
...
//      via the `UnicodeWidthLibrary` product.
//
...
final class UnicodeWidthExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "UnicodeWidth swift module imported cleanly")
    }
}
